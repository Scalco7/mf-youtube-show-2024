...
export class FavoriteController {
    ...
    public async listFavoriteVideosByUserId(data: { userId: string }): Promise<string[]> {
        const { error } = listFavoritesDataValidator.validate(data)

        if (error)
            throw (error.message)

        const favorites = await this.favoriteService.listFavoriteVideosByUserId(data.userId)
        return favorites
    }

    public async addFavoriteVideo(data: IFavoriteVideoRequestData): Promise<void> {
    ...
    public async removeFavoriteVideo(data: IFavoriteVideoRequestData): Promise<void> {
        const { error } = favoriteDataValidator.validate(data)
